...
#ifndef DOCUMENTSCORERREF_HPP
#define DOCUMENTSCORERREF_HPP
...
public:
    ...
    DocumentScorerRef(const DocumentScorer ds_ins)
    {
        ...
        pq_codes = ds_ins.pq_codes;

        centroids = ds_ins.centroids;
        n_centroids = ds_ins.n_centroids;
        cout << "Number of Centroids:  " << n_centroids << "\n";

        centroids_assignments = ds_ins.centroids_assignments;
        centroids_scores.resize(M * n_centroids);
        centroids_scores_transposed.resize(M * n_centroids);

        all_doclens = ds_ins.all_doclens;
        n_docs = ds_ins.n_docs; // number of documents in the entire collection
        K = ds_ins.K;
        tot_embedding = ds_ins.tot_embedding;
        ...
        size_t ntotal = ds_ins.pqCodesArray.shape[0];

        vector<float> pqcentroids{ds_ins.pqCentroidsArray.data<float>(), ds_ins.pqCentroidsArray.data<float>() + ds_ins.pqCentroidsArray.shape[0]};
        pq = ProductQuantizerX(K, ds_ins.pqCodesArray.shape[1], nbits, pq_codes, pqcentroids);

        centroids_to_pid = ds_ins.centroids_to_pid;
